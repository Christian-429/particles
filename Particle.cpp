    ...
    :m_A(2,numPoints), m_ttl(TTL), m_numPoints(numPoints)
{
    ...
    float thetaStart = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
    float theta = thetaStart * (static_cast<float>(M_PI) / 2.0f);
...
void Particle::draw(RenderTarget& target, RenderStates states) const
{
    ...
    for (int j = 1; j <= m_numPoints; j++)
    {
        ...
        Vector2f cartesianPoint(x, y);
        Vector2f pixelPoint = static_cast<Vector2f>(target.mapCoordsToPixel(cartesianPoint, m_cartesianPlane));
